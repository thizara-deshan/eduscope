...
// Lmc
export const ApplyLmc = async (req, res) => {
    ...
    try {
        ...
        let allok = new Promise((resolve, reject) => {
            ...
            dataArr.forEach((data, index, array) => {
                Lmc.updateById(req.params.id, data, (err, data) => {
                    if (err) {
        ...
        return res.status(500).json({
            message: "Unable to create Lmc.",
            success: false
        });
    }
};

export const GetLmc = (req, res, next) => {

    Lmc.getAll(req.params.id, (err, data) => {
        if (err)
            res.status(500).send({
                message:
