...
Settings.findById = (id, result) => {
    ...
    sql.query(`SELECT * FROM settings WHERE id = ${id} AND submenu='ss'`, (err, res) => {
        if (err) {
            console.log("error: ", err);
            result(err, null);
            return;
...
Settings.getAll = (userid, result) => {
    let query = `SELECT * FROM settings WHERE userid = ${userid} AND submenu='ss'`;
...
Settings.updateById = (id, setting, result) => {
    ...
    sql.query(
        `UPDATE settings SET s_value = ?, updated_at = '${updated_at}' WHERE userid = ${id} AND title = ? AND submenu='ss'`,
        [setting[1], setting[0]],
        (err, res) => {
            if (err) {
                console.log("error: ", err);
