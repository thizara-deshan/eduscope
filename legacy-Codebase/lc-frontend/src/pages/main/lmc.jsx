...
import { Button as ButtonAnt, Input as InputAnt, message, Divider, Select, PageHeader, Layout, Alert, Switch, Slider, Modal } from 'antd';
...
const Lmc = () => {
    ...
    useEffect(async () => {
        ...
        if ((BEDevices.filter(x => x == "Eduscope UMS ").length) == 1) {
            if (lmcOptions.presentation_source == 'hdmisource' || lmcOptions.presentation_source == 'sdisource') {

            } else if (lmcOptions.presentation_source == 'rtsp') {
                if (!rtspOn) {
                    setLmcOption({ ...lmcOptions, presentation_source: 'Select a Device' })
            ...
            } else if (lmcOptions.presentation_source == 'rtsp2') {
                if (!rtspOn2) {
                    setLmcOption({ ...lmcOptions, presentation_source: 'Select a Device' })
                    socket.removeAllListeners();
                    setDeviceMissing(true)
                ...
                if (BEDevices.length != 2) {
                    setLmcOption({ ...lmcOptions, presenter_source: 'Select a Device', presentation_source: 'Select a Device' });
                    socket.removeAllListeners();
                    setDeviceMissing(true)
                    setImgs1(placeholder)
                    setImgs2(placeholder)
            ...
            if (lmcOptions.presenter_source == 'hdmisource' || lmcOptions.presenter_source == 'sdisource') {

            } else if (lmcOptions.presenter_source == 'rtsp') {
                if (!rtspOn) {
                    setLmcOption({ ...lmcOptions, presenter_source: 'Select a Device' })
                    socket.removeAllListeners();
                    setDeviceMissing(true)
                    setImgs1(placeholder)
                    setImgs2(placeholder)
            ...
            } else if (lmcOptions.presenter_source == 'rtsp2') {
                if (!rtspOn2) {
                    setLmcOption({ ...lmcOptions, presenter_source: 'Select a Device' })
                    socket.removeAllListeners();
                    setDeviceMissing(true)
                ...
                if (BEDevices.length != 2) {
                    setLmcOption({ ...lmcOptions, presentation_source: 'Select a Device', presenter_source: 'Select a Device' })
                    setDeviceMissing(true)
                    socket.removeAllListeners();
                    setImgs1(placeholder)
                    setImgs2(placeholder)
    ...
    const getDeviceList = async () => {
        ...
        await CaptureSetupCtrl.getDevices('video').then((res) => {
            ...
            if (res.data) {
                message.success({ content: 'Video Sources Loaded', key, duration: 1 })
            } else {
    ...
    const switchpresentation = (presenter, value) => {
        ...
        CaptureSetupCtrl.CsChangeSnaps(source, { rtspOptions }).then((result) => {
            console.log(result);
            if (deviceMissing && (lmcOptions.presenter_source != 'Select a Device')) { setDeviceMissing(false) }
    ...
    const switchpresenter = (presentation, value) => {
        ...
        CaptureSetupCtrl.CsChangeSnaps(source, { rtspOptions }).then((result) => {
            console.log(result);
            if (deviceMissing && (lmcOptions.presentation_source != 'Select a Device')) { setDeviceMissing(false) }
            message.success({ content: 'Video Source Loaded!', key, duration: 1 })
            setIsLoad(false)
        }).catch((err) => {
            console.log(err);
    ...
    // useEffect(() => {
    //     getVideo(vidRefPresentation, lmcOptions.presentation_source);
    // }, [lmcOptions.presentation_source]);

    // useEffect(() => {
    //     getVideo(vidRefPresenter, lmcOptions.presenter_source);
    // }, [lmcOptions.presenter_source]);
    ...
    const apply = () => {
        ...
        setIsLoad(true)
        LmcCtrl.LmcApply(lmcOptions).then((res) => {
            if (res.success) {
    ...
    const handleDropDowns = (value, event) => {
        setApplyVal(true)
        setLmcOption({ ...lmcOptions, [event.name]: value });
        if (event.name == "presentation_source") {
            switchpresentation(lmcOptions.presenter_source, value)
        }
        if (event.name == "presenter_source") {
            switchpresenter(lmcOptions.presentation_source, value)
        }
                                ...
                                <div className="w-100">
                                    <Col lg={10} className="bg-white maincontainer mx-auto shadow rounded-3 nopadblock2">
                                        <Layout className="bg-white rounded-3">
                                            <Header className="p-0 h-auto">
                                                <div className="site-page-header-ghost-wrapper">
                                                    <PageHeader
                                                        ...
                                                        ghost={false}
                                                        title="Eduscope UMS"
                                                        subTitle="Live Meeting Connect"
                                                        extra={[
                                                            <ButtonAnt key="1" className='cus-btn1' disabled={isLoad} type="default" onClick={apply}>
                                                                Apply
                                                            </ButtonAnt>,
                                                            <ButtonAnt key="2" className="reset-btn cus-btn1" disabled={isLoad} type="dashed" onClick={() => { console.log("Reset"); }}>
                                                                Reset
                                                            </ButtonAnt>,
                                                            <ButtonAnt key="3" className='cus-btn1' type="primary" disabled={isLoad} onClick={() => {
                                                                if (applyVal) {
                                                                    confirm({
                                            ...
                                            <Content className="" style={{ padding: '10px 50px' }}>
                                                <Row>
                                                    <Col lg={6}>
                                                        <div className={lmcOptions.layout == "Presenter" ? "d-none" : ""}>

                                                            <Divider orientation="left" style={{ borderColor: "#121212", fontWeight: "900" }}>Presentation Preview</Divider>
                                                            <img className={lmcOptions.presentation_source == '' || privPipe ? "empty-vid" : "recordpiv"} src={imgs1}></img>
                                                            <div>
                                                                <Select disabled={isLoad} className="my-3" placeholder="Presentation Source" value={lmcOptions.presentation_source} style={{ width: "100%" }} onSelect={(value, event) => handleDropDowns(value, event)}>
                                                            ...
                                                            {/* <video className={lmcOptions.presentation_source == '' ? "empty-vid" : "recordpiv"} ref={vidRefPresentation}></video> */}
                                                            {/* <h6 className="text-center ">Presentation Preview</h6> */}
                                                        </div>

                                                    </Col>
                                                    <Col lg={6}>
                                                        <div className={lmcOptions.layout == "Presentation" ? "d-none" : ""}>

                                                            <Divider orientation="left" style={{ borderColor: "#121212", fontWeight: "900" }}>Presenter Preview</Divider>
                                                            <img className={lmcOptions.presenter_source == '' || privPipe ? "empty-vid" : "recordpiv"} src={imgs2}></img>

                                                            <div>
                                                                <Select disabled={isLoad} className="my-3" placeholder="Camera Source" value={lmcOptions.presenter_source} style={{ width: "100%" }} onSelect={(value, event) => handleDropDowns(value, event)}>
                                                            ...
                                                            {/* <video className={lmcOptions.presenter_source == '' ? "empty-vid" : "recordpiv"} ref={vidRefPresenter}></video> */}
                                                ...
                                                <Row>
                                                    <Col lg={12}>
                                                        <Divider orientation="left" style={{ borderColor: "#121212", fontWeight: "900" }}>Layout</Divider>
                                                    </Col>
                                                    <Col lg={6}>
                                                        <div >
                                                            <Col lg={12} sm={6}>
                                                                <Select disabled={isLoad} className="my-3" placeholder="Layout" value={lmcOptions.layout} style={{ width: "100%" }} onSelect={(value, event) => handleDropDowns(value, event)}>
                                                            ...
                                                            <Col lg={6} sm={6} >
                                                                <div className="d-grid">
                                                                    <div className={layoutVal == "PresentationPresenter" ? "d-inline-block mt-2" : "d-none"}>
                                                                        <Select disabled={isLoad} className="my-2" placeholder="Record Method" value={lmcOptions.rec_method} style={{ width: "100%" }} onSelect={(value, event) => handleDropDowns(value, event)}>
                                                                ...
                                                                <Col lg={12}>
                                                                    <div className={pipval == "PIP" && layoutVal == "PresentationPresenter" ? "px-1 mt-2" : "d-none"}>
                                                                        <Select disabled={isLoad} className="my-2" placeholder="PIP Position" value={lmcOptions.pip_pos} style={{ width: "100%" }} onSelect={(value, event) => handleDropDowns(value, event)}>
                                                                    ...
                                                                    <div className={pipval == "Side" && layoutVal == "PresentationPresenter" ? "px-1 mt-2" : "d-none"}>
                                                                        <Select className="my-2" placeholder="PIP Position" value={lmcOptions.s_pip_pos} style={{ width: "100%" }} onSelect={(value, event) => handleDropDowns(value, event)}>
                                                                ...
                                                                <Col lg={12}>
                                                                    <div className={(pipval == "PIP" || pipval == "Side") && layoutVal == "PresentationPresenter" ? "px-1 mt-2" : "d-none"}>
                                                                        <Select className="my-2" placeholder="PIP Size" value={lmcOptions.pip_size} style={{ width: "100%" }} onSelect={(value, event) => handleDropDowns(value, event)}>
