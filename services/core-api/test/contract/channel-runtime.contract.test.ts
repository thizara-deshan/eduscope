...
import { zDisableChannelResponse, zEnableChannelResponse, zProblem } from '@eduscope/shared';
...
const BEARER = 'contract-test-pm-bearer-channels';
...
async function startTestApp(): Promise<TestApp> {
  const dir = mkdtempSync(join(tmpdir(), 'core-api-channel-runtime-contract-'));
  ...
  const config = loadConfig({
    ...
    CORE_API_JWT_SECRET: 'channel-runtime-contract-secret',
...
async function startAndConfirmSession(testApp: TestApp): Promise<void> {
...
describe('channel runtime contract (openapi.yaml tag: channels — enableChannel, disableChannel)', () => {
  let testApp: TestApp;
  ...
  it('enableChannel: 202 parses zEnableChannelResponse', async () => {
    testApp = await startTestApp();
    await startAndConfirmSession(testApp);

    const response = await testApp.app.inject({
      method: 'POST',
      url: '/api/v1/channels/meeting/enable',
      headers: { authorization: `Bearer ${testApp.ownerToken}` },
    });

    expect(response.statusCode).toBe(202);
    expect(() => zEnableChannelResponse.parse(response.json())).not.toThrow();
  });

  it('enableChannel: 409 session.not-active parses zProblem when nothing is recording', async () => {
    testApp = await startTestApp();
  ...
  it('enableChannel: 422 config.invalid parses zProblem for the local channel', async () => {
    testApp = await startTestApp();
    await startAndConfirmSession(testApp);

    const response = await testApp.app.inject({
      method: 'POST',
      url: '/api/v1/channels/local/enable',
      headers: { authorization: `Bearer ${testApp.ownerToken}` },
    });
  ...
  it('disableChannel: 202 parses zDisableChannelResponse', async () => {
    testApp = await startTestApp();
    await startAndConfirmSession(testApp);
    ...
    const response = await testApp.app.inject({
      method: 'POST',
      url: '/api/v1/channels/meeting/disable',
      headers: { authorization: `Bearer ${testApp.ownerToken}` },
    });

    expect(response.statusCode).toBe(202);
    expect(() => zDisableChannelResponse.parse(response.json())).not.toThrow();
  });
});
