...
**Interfaces:**
- Produces: `loadConfig(env): CoreConfig`; `buildApp(deps?): Promise<FastifyInstance>`; `ProblemError`; `parseBody(schema, value)`; `assertV1Contracts()`.
...
`CoreConfig` must include localhost host/port, DB path, recordings root, runtime dir, provisioning path, helper socket, pipeline-manager base URL `http://127.0.0.1:8091`, internal bearer, JWT key, secretbox key, and test-injectable access/refresh TTLs. `buildApp()` installs one error handler that converts `ProblemError` and `ZodError` to `application/problem+json` and registers public `GET /healthz → {status:"ok",contractVersion:"1.0.0"}`. `server.ts` loads config, builds the app, awaits startup, and listens only on the validated host.
...
Expected: PASS; 78/4/22 counts and all v1 version assertions are green.
...
- Consumes: `CoreConfig.dbPath`, shared entity types, shared layout catalog.

- [ ] **Step 1: Write real-SQLite failing tests**

Use a temp DB. Assert PRAGMAs are `journal_mode=wal`, `foreign_keys=1`, `synchronous=1` (`NORMAL`), `busy_timeout=5000`; migrations can be called twice; seed counts are 5 source roles, 7 layout presets, 3 channel configs, 4 physical-input skeletons/bindings, one retention policy, and one device-default encoding profile. Race two writes and prove the partial unique indexes reject a second non-terminal `lecture_sessions.device_id`, a second showing publication per quiz session, and a second mounted recordings volume.
...
Migrations run before route registration/listen and abort startup on failure. Long work must never execute inside `db.transaction()`.

- [ ] **Step 4: Run DB, type, and contract regressions**

Run: `pnpm --filter @eduscope/core-api test -- test/db/migrations.test.ts test/db/invariants.test.ts`

Expected: PASS; second migration run is a no-op and all four uniqueness races reject exactly one contender.
...
The scheduler wakes every 5 seconds and on row changes, selects one due queued job, and delegates transfer outside transactions. Persist structural `failureClass`; never parse `lastError`. `requeue` resets terminal retry fields and records actor/time but does not create a second job.
...
Use TLS verification for non-loopback URLs; B-18 tests only the loopback fixture. Persist each checkpoint before reading the next chunk. This is not institute-wire acceptance.
...
**Interfaces:**
- Produces: fixed-union `HelperClient.request(verb,args,requestId)`; `registerStorageVolume`, `formatStorageVolume`.
- Consumes: helper Unix socket fixture, current block-device resolver, recording guard.

- [ ] **Step 1: Write failing helper/volume safety tests**

Reject unknown verbs, extra args, non-current devnodes, duplicate UUID, wrong confirm text, non-admin, non-terminal recording, helper timeout/failure, and path-like metacharacters. Register uses `volume.mount {uuid}`; format uses only `volume.unmount`, `volume.format {devNode,fs:'ext4',label}`, `volume.mount {uuid}` and records by UUID.
...
Validate verb/args with a closed zod discriminated union, connect to `/run/eduscope/helper.sock`, write `{verb,args,requestId}\n`, read one bounded line, validate `{ok,detail}`, and close. No shell, sudo, generic exec, or caller-supplied executable exists.

- [ ] **Step 4: Verify only allowlisted calls occur**

Run: `pnpm --filter @eduscope/core-api test -- test/storage/volumes.test.ts test/contract/storage-volumes.contract.test.ts`

Expected: PASS; fake helper ledger contains only exact fixed verbs/argv-like data.
...
Prove unsupported values are absent/rejected; GET without `channelId` returns device default, with channel returns override or inherited default; update absent/null `channelId` writes default, non-null writes only that channel; bitrate 2000–8000 Kbps; and a streaming update converts Kbps→Bps into the next PM live start while local record retains its default. Passthrough record receives no active override effect.
...
Expected: PASS; fake A captures the changed streaming values and unchanged local values.
...
Cover YouTube/Facebook/custom RTMP validation, write-only stream key, `hasStreamKey` only in responses, no secret in DB/log/audit/template snapshot, stable sorted digest, reload only on effective change, and edits during active stream update next-run config without stopping recording/current live consumer.
...
Expected: PASS; PM ledger contains no stop call after target edit.
...
4. Pause, resume, and stop; assert two finalized segments, one logical recording, channels isolated, and all resolving events arrive within configured fake deadlines.
5. Fake ffprobe/ffmpeg completes merge; assert library owner scope, authenticated 206 media read, and ready artifact.
6. Insert two USB targets; export to the selected one, observe scoped real-byte progress, and complete.
7. Let placeholder upload disconnect mid-part; restart core-api, resume the durable offset, complete all parts, and observe queued→uploading→done.
8. Reconnect WS; assert full snapshot, monotonic new seq, no duplicate upload/answer projection, and private export/log events absent from another AuthSession.
9. Log out; assert the token/session can no longer use REST or either socket.
...
Expected: PASS; evidence contains login→record→pause/resume→stop→library/export/upload, ordered WS seq, 206 media proof, restart/resume proof, and zero unhandled contract elements.
...
- KEEP coverage is preserved at its master task: B-03 (B-05), B-07/B-10 (B-06), B-15 (B-07), B-23 (B-13), B-31/B-33 (B-14), B-32 (B-16), B-27/B-28 (B-17), B-53 (B-19), B-50 (B-22), B-43 (B-27), B-44 (B-28).
