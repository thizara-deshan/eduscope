...
- **Demo/staging kiosk exception (approved 2026-09-10; three-display mapping updated 2026-09-17):** F-07 may run with `--profile demo-staging` while the final production display identities remain unavailable. It requires exactly the currently connected `HDMI-1`, `DP-2`, and `DP-1` outputs: `HDMI-1` is the temporary projector at `0x0`, `DP-2` is the full-screen meeting output immediately to its right, and the new `DP-1` display is the 1920×1080 primary kiosk panel at the far right with the frozen touch device mapped only to it. Production remains fail-safe and requires exactly one match for each of the three frozen EDID hashes before changing layout. This connector-bound demo smoke never satisfies F-07's deterministic EDID mapping, final 1280×800 panel, or two-cold-boot acceptance witness.
...
- [ ] **Step 4: Implement fail-safe EDID placement and touch mapping**
  ...
  For the approved demo/staging exception, `--profile demo-staging` requires exactly three connected outputs named `HDMI-1`, `DP-2`, and `DP-1`. It configures the 1920×1080 projector on `HDMI-1` at `0x0`, the 1920×1080 full-screen meeting output on `DP-2` at `1920x0`, and the new 1920×1080 kiosk panel on `DP-1` at `3840x0` as primary; maps the exact manifest touch name to `DP-1`; and prints `multi-display acceptance open`. Any other active topology fails before mutation. This is demo smoke only and does not close the production witness.
...
- [ ] **Step 6: Run automated and cold-boot acceptance**
  ...
  On the current demo target, defer the live layout and Chromium checks to F-08 Step 7, because F-08 installs and activates the panel, Nginx, GDM session, and kiosk unit that those checks consume. F-08 runs the layout with `--profile demo-staging`, verifies Chromium on the primary `DP-1` kiosk output, and requires the script to print `multi-display acceptance open`. Record the production EDID/final-panel and two-cold-boot result as `NOT RUN`; the demo smoke does not close that physical acceptance.
