...
| **CG-5** | **`GET /recordings` cannot be filtered by owner, date or title.** Params are `cursor`, `limit`, `state`, `includeDeleted`. An admin looking at every lecturer's 14 days of recordings has scroll as the only tool | S-21 | Low | Add `?q=`, `?ownerUserId=`, `?from=`/`?to=` to `listRecordings` | ✅ **answered** — a **scoped subset**: add `?q=` (title) and `?ownerUserId=` (admin) only; **not** `?from=`/`?to=` (the 14-day window is already small). Chips, not a menu; server-side, not client-filtered. To be **applied v0.5** before Wave 5's plan run | [S-21-design.md](screens/S-21-design.md) §9 |
