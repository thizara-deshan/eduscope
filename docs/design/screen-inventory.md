...
| W-5 | **S-21** Recordings library | Parity §5.1 item 1 — the largest gap in the product | S-22, S-23, S-24 | ✅ **closed** 2026-08-09 | [S-21-design.md](screens/S-21-design.md) |
| W-6 | **S-22** Recording detail & player | Same row; authenticated playback is new (B-37) | — | ✅ **closed** 2026-08-09 | [S-22-design.md](screens/S-22-design.md) |
...
| **CG-5** | **`GET /recordings` cannot be filtered by owner, date or title.** Params are `cursor`, `limit`, `state`, `includeDeleted`. An admin looking at every lecturer's 14 days of recordings has scroll as the only tool | S-21 | Low | Add `?q=`, `?ownerUserId=`, `?from=`/`?to=` to `listRecordings` | ✅ **applied v0.5.0** | [S-21-design.md](screens/S-21-design.md) §9 |
...
| **CG-22** | **The student realtime channel has event names but no transport/auth/resync contract.** There is no declared student WS URL, participant authentication, reconnect behavior or atomic snapshot ordering | S-39, S-40, S-41 | **Blocking** — reconnect can flash or retain stale question/result state | Define participant-cookie-authenticated student WS plus an atomic full snapshot on every connect before live deltas | ✅ **applied v0.6.0** | [S-39-design.md](screens/S-39-design.md) §8 |
| **CG-23** | **`quiz.question{state:none}` is impossible in the current schema.** `publicationId`, prompt and options remain required for every state, and `ownAnswer` does not say whether it is an answer id or option id | S-39 | **Blocking** for waiting and reconnect | Replace with state-discriminated `open/closed/none` variants and name `ownAnswerOptionId` | ✅ **applied v0.6.0** | [S-39-design.md](screens/S-39-design.md) §8 |
| **CG-24** | **`quiz.result` is not self-contained.** It lacks prompt/options, the student's selected option and rank freshness, so S-40 cannot reveal answer text or distinguish `rank updating` after cold load/reconnect | S-40 | **Blocking** for the complete result states | Add question snapshot, `selectedOptionId` and `rankState` to the own-result payload | ✅ **applied v0.6.0** | [S-40-design.md](screens/S-40-design.md) §7 |
| **CG-25** | **Closed `quiz.session` final fields are all nullable with no participation discriminator.** A valid zero/never-answered summary is indistinguishable from incomplete data | S-41 | **Blocking** for a trustworthy terminal summary | Make the payload state-discriminated; closed carries `participationState` and constrained final fields | ✅ **applied v0.6.0** | [S-41-design.md](screens/S-41-design.md) §7 |

Gaps **CG-1** and **CG-2** are the two that hard-block a build wave *by
missing an entire surface*. **CG-1 and the student-event gaps CG-22…CG-25 are
now `applied v0.6.0`** ([contract-amendments.md](contract-amendments.md),
2026-08-11), so Wave 7's plan run is unblocked. **CG-10…CG-13 blocked Wave 1** and are now
...
(2026-08-05)** and are now `applied v0.3.0` — contract, zod layer and mock
adapter all carry them ([contract-amendments.md](contract-amendments.md),
2026-08-05), so Wave 2's plan run is unblocked; **CG-6 closed as a confirm**
...
run is unblocked. **The W-5…W-9 gate (2026-08-09) answered CG-3, CG-5 and CG-7
and added two rows — CG-20** (`UploadJob.failureClass`, S-35) **and CG-21**
(`export.insufficient-space`, S-23); all five ride the **v0.5** bump before Wave
5's plan run. **S-24 (W-8) required zero contract change** — the wave's clean
"a run can add nothing" case, in the W-14/W-15 style.
...
| `v0.6` | CG-1, CG-22, CG-23, CG-24, CG-25 | before Wave 7 |
...
| SI-D-3 | Library entry point | **Taken (W-5, 2026-08-09):** a header entry visible to both roles, plus a link from the post-stop "Saved" toast. See [S-21-design.md](screens/S-21-design.md) §11 LIB-D-6 | Low |
