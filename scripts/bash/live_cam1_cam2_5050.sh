#!/bin/bash
# cam1 + cam2 side-by-side + audio -> local nginx RTMP.
#   ./live_cam1_cam2_5050.sh [A] [B] [KEY]
source "$(dirname "$0")/_layout.sh"
A=${1:-50}; B=${2:-50}; KEY="${3:-test}"
TARGET="rtmp://127.0.0.1:1935/live/$KEY"
eval $(ratio_layout $A $B); echo "LIVE cam1/cam2 ${A}/${B} -> $TARGET"
gst-launch-1.0 -e \
